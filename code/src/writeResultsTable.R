...
write.ms.table.differences <- function(mod.output, mod.name){
    ...
    for(j in (i+1):length(levels)){
      ...
      if(coeff_name1 %in% colnames(samps.mod) & coeff_name2 %in% colnames(samps.mod)){
        ...
        diff.results <- rbind(diff.results, data.frame(Comparison = paste(level1, "-", level2),
                                                       ...
                                                       Plt0 = round(p_lt_0, 4),
                                                       BayesP = bayes_p,
                                                       Pgt0Stars = pstars2(p_gt_0),
                                                       Plt0Stars = pstars2(p_lt_0)))
      }
    }
  }
  ...
  sum.mod$Pgt0Stars  <- sapply(sum.mod$Pgt0, pstars2)
  sum.mod$Plt0Stars  <- sapply(sum.mod$Plt0, pstars2)
...
# pstars for multiple comparisons
pstars2 <- function(p) {
  if (p < 0.001) return("***")
  if (p < 0.01) return("**")
  if (p < 0.05) return("*")
