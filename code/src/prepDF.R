...
calculateLandscapeMetrics <- function(landcover.raster, site.shapefile, landcover.classification, buffer.sizes = c(500)){
  ...
  for (buffer in buffer.sizes){
    ...
    lsm_ca <- sample_lsm(landcover.raster,
                                y = site.shapefile,
                                ...
                                what = "lsm_c_ca")
    #set class IDs
    lsm_ca = merge(lsm_ca, landcover.classification, by = "class")
    colnames(lsm_ca)[colnames(lsm_ca) == "plot_id"] <- "sample_pt"
    
    #shannon diversity
    lsm_l_shdi <- sample_lsm(landcover.raster,
                                  y = site.shapefile,
                                  plot_id = site.shapefile$site_id,
                                  shape = "circle",
                                  size = buffer,
                                  return_raster = TRUE,
                                  what = "lsm_l_shdi")
    res1_shdi = lsm_l_shdi[,c("plot_id", "value")]
    ...
    buffer_areas = lsm_ca %>% 
      group_by(sample_pt) %>%
      summarise(buffer_area=sum(value))
    lsm_ca = merge(lsm_ca, buffer_areas, by = "sample_pt")
    res1_blueberry = filter(lsm_ca, lc_type == "blueberry")
    ...
    res1_edge = filter(lsm_ca, lc_type == "blackberry" | lc_type == "lowedge" | lc_type == "hedgerow")
    res1_edge$split_prop = res1_edge$value/res1_edge$buffer_area
    res1_edge_summary = res1_edge %>%
      group_by(sample_pt) %>%
      summarize(prop_edge = sum(split_prop))
    ...
    # fill NAs with zeroes -- this occurs when that landcover class doesn't exist in that buffer area so it's a true zero
    landscape.dat[is.na(landscape.dat)] = 0
