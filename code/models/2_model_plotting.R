...
load(file="saved/ParasiteModels_interaction.Rdata")
load(file="saved/ParasiteModels_nativebombus.Rdata")


# compute & save and/or load conditional effects
base.cond.effects <- conditional_effects(fit.bombus.all)
inter.cond.effects <- conditional_effects(fit.bombus.inter)
native.cond.effects <- conditional_effects(fit.bombus.native)

save(all.cond.effects, inter.cond.effects, native.cond.effects,
...
hascrith <-
  inter.cond.effects[["hascrithidia.hascrithidia_impatiens_abundance:status"]]
...
hascrith.imp.status <- 
  ...
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = status,
                  alpha=1)) +
  geom_line(aes(x = impatiens_abundance, y=estimate__, linetype = status), linewidth = 1.5) +
...
## ***********************************************************************
hascrith <-
  inter.cond.effects[["hascrithidia.hascrithidia_native_bee_abundance:status"]]
...
hascrith.babund.status <- 
  ...
  geom_line(aes(x = native_bee_abundance, y=estimate__, linetype = status), linewidth = 1.5) +
  scale_fill_manual(values = c("black", "lightblue")) +
  ...
  theme(axis.title.x = ggtext::element_markdown(size = 16), #change to element_blank() for grid plots!
        axis.title.y = ggtext::element_markdown(size=16),
        text = element_text(size=16))
  #geom_text(data = data.frame(x = c(35),y = c(1.1),label = c("**")), aes(x=x, y=y, label=label), color = "black", fontface = "bold", size=12)
...
## ***********************************************************************
## has nosema ~ impatiens abundance (in just native Bombus)
## ***********************************************************************

hasnosema <-
  native.cond.effects[["hasnosema.hasnosema_impatiens_abundance"]]

#ggplot
hasnos.imp.native <- 
  
  #plot model prediction with credible interval
  ggplot(hasnosema, aes(x = impatiens_abundance, y = estimate__), color = "black") + 
  geom_ribbon(aes(ymin = lower__, ymax = upper__, alpha=1), fill = "black") +
  geom_line(aes(x = impatiens_abundance, y=estimate__), linewidth = 1.5, linetype = "dashed") +
  #scale_fill_manual(values = c("black", "lightblue")) +
  #scale_colour_manual(values = c("black", "lightblue")) +
  ...
  labs(x = "", y = "*Vairimorpha spp.* prevalence") +
  theme_ms() +
  theme(legend.position = "none") +
  scale_x_continuous(limits = c(0,11)) +
  scale_y_continuous(limits = c(0,1)) +
...
## ***********************************************************************
## has nosema ~ native bee abundance (in just native Bombus)
## ***********************************************************************

hasnosema <-
  native.cond.effects[["hasnosema.hasnosema_native_bee_abundance"]]

#ggplot
hasnos.babund.native <- 
  
  #plot model prediction with credible interval
  ggplot(hasnosema, aes(x = native_bee_abundance, y = estimate__), color = "black") + 
  geom_ribbon(aes(ymin = lower__, ymax = upper__, alpha=1), fill = "black") +
  geom_line(aes(x = native_bee_abundance, y=estimate__), linewidth = 1.5, linetype = "dotted") +
  labs(x = "", y = "") +
  theme_ms() +
  theme(legend.position = "none") +
  scale_y_continuous(limits = c(0,1)) +
...
## ***********************************************************************
## apicystis ~ impatiens abundance x status
## ***********************************************************************
apicystis <-
  inter.cond.effects[["apicystis.apicystis_impatiens_abundance:status"]]

apicystis.imp.status <- 
  
  #plot model prediction with credible interval
  ggplot(apicystis, aes(x = impatiens_abundance, y = estimate__, color = status)) + 
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = status,
                  alpha=1)) +
  geom_line(aes(x = impatiens_abundance, y=estimate__), linetype = "dotted", linewidth = 1.5) +
  scale_fill_manual(values = c("black", "lightblue")) +
  scale_colour_manual(values = c("black", "lightblue")) +
  #scale_color_manual(values = c("black", "blue")) +
...
## ***********************************************************************
## apicystis ~ native bee abundance x status
## ***********************************************************************
apicystis <-
  inter.cond.effects[["apicystis.apicystis_native_bee_abundance:status"]]
apicystis = apicystis %>% filter((status == "native") | (native_bee_abundance <= max(data.par$native_bee_abundance[data.par$final_id == "Bombus_impatiens"])))


apicystis.babund.status <- 
  
  #plot model prediction with credible interval
  ggplot(apicystis, aes(x = native_bee_abundance, y = estimate__, color = status)) + 
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = status,
                  alpha=1)) +
  geom_line(aes(x = native_bee_abundance, y=estimate__), linewidth = 1.5, linetype = "dotted") +
  scale_fill_manual(values = c("black", "lightblue")) +
  ...
  #plot raw data
  #geom_jitter(data = data.par, aes(x=impatiens_abundance,y = hascrithidia), height = 0.05) +
  labs(x = "Native *Bombus* abundance", y = "") +
  theme_ms() +
  theme(legend.position = "none") +
  scale_y_continuous(limits = c(0,1)) +
  theme(axis.title.x = ggtext::element_markdown(size = 16), #change to element_blank() for grid plots!
...
interactions <- grid.arrange(
  arrangeGrob(hascrith.imp.status, hascrith.babund.status,
              hasnos.imp.native, hasnos.babund.native,
              apicystis.imp.status, apicystis.babund.status,
              ncol = 2),
...
#add labels
interactions <- ggdraw() +
  draw_plot(interactions, 0.015, 0, 1, 1) +
  draw_plot_label(c("(A)", "(B)", "(C)", "(D)", "(E)", "(F)"), 
                  x = c(0, 0.5, 0, 0.5, 0, 0.5), 
                  y = c(1, 1, 0.7, 0.7, 0.4, 0.4))
#add labels
ggsave(filename = "figures/manuscript_figures/interactions.jpg", 
       plot = interactions, 
       width = 3000, 
       height = 4000, 
       units = "px")
